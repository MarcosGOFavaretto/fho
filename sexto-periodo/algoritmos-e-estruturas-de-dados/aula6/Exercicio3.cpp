...
void adicionarDocumento(Queue &q, const char nomeArquivo[], int numPaginas, int tempoEstimado)
{
    ...
    // Usando cin para capturar até o limite
    int i = 0;
    while (i < LIMITE_CARACTERE - 1 && nomeArquivo[i] != '\0')
    {
        q.queue[q.fim].nomeArquivo[i] = nomeArquivo[i];
        i++;
    }
    q.queue[q.fim].nomeArquivo[i] = '\0'; // Garantir que o nome do arquivo seja terminado com '\0'
